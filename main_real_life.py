"""
Real-Life 数据集 - pplg_muti 非对称多模态模型训练脚本
LOOCV 留一法交叉验证，最多 30 epoch，预测正确则早停
"""
import os, glob
...
from models.pplg_new import LieDetection
...
from utils.train import train_muti_real_epoch
...
def main():
    ...
    logger.info("=" * 60 + "\n Starting Real-Life pplg_muti LOOCV (max 30 epochs, early stop if correct)\n" + "=" * 60)
    ...
    for fold_idx in range(num_total_samples):
        ...
        for epoch in range(max_epochs):
            ...
            train_metrics = train_muti_real_epoch(model, train_loader, criterion_bc, optimizer, epoch, args)
    ...
    report_lines = []
    report_lines.append("\n" + "=" * 80)
    report_lines.append(" FINAL AGGREGATED LOOCV RESULTS (pplg_muti)")
