...
class LieDetection(nn.Module):
    ...
    包含: 内容直连、辅助分类器、通道级 ReZero、温度锐化、梯度截断、Audio Drop、多种聚合方式
    """
    def __init__(self, args, initial_prototypes=None):
        ...
        # [新增] 获取配置中的聚合方式，默认为原先的 hierarchical_max
        self.aggr_method = getattr(args, 'aggr_method', 'hierarchical_max')
        
        # [修改] 根据聚合方式动态决定单分支特征的维度
        if self.aggr_method in ['hierarchical_max', 'hierarchical_mean']:
            self.single_branch_dim = args.low_dim * args.num_classes  # K * D
        elif self.aggr_method in ['mean', 'max']:
            self.single_branch_dim = args.low_dim                     # D
        else:
            raise ValueError(f"未知的聚合方式: {self.aggr_method}。可选值: 'hierarchical_max', 'hierarchical_mean', 'mean', 'max'")
        ...
        if self.modality == 'both':
            ...
            # 投影音频引导的视觉特征 (维度自动适配 single_branch_dim)
            ...
            # 投影音频纯内容特征 (维度自动适配 single_branch_dim)
    ...
    def _aggregate_visual_features(self, features_list):
        """
        [修改] 统一的视觉特征聚合函数，支持多种聚合方式。
        替换了原先硬编码的 _hierarchical_aggregate。
        """
        num_instances_per_bag = [f.shape[0] for f in features_list]
        features_flat = torch.cat(features_list, dim=0)

        # 无论哪种聚合方式，都需要先通过实例分类器获取实例级特征
        ...
        # 策略 1: 直接 Mean Pooling 或 Max Pooling (不区分簇)
        if self.aggr_method in ['mean', 'max']:
            for embeddings in instance_embeddings_list:
                if self.aggr_method == 'mean':
                    bag_feature = torch.mean(embeddings, dim=0)
                else: # max
                    bag_feature, _ = torch.max(embeddings, dim=0)
                final_bag_features.append(bag_feature)
            
            bag_features = torch.stack(final_bag_features, dim=0)

        # 策略 2: 层次化聚合 (按原型聚类后进行簇内 Pooling 并拼接)
        elif self.aggr_method in ['hierarchical_max', 'hierarchical_mean']:
            ...
            for i in range(len(instance_embeddings_list)):
                ...
                for k in range(self.args.num_classes):
                    ...
                    if len(indices_in_cluster) == 0:
                        ...
                        if self.aggr_method == 'hierarchical_max':
                            cluster_feature, _ = torch.max(embeddings[indices_in_cluster], dim=0)
                        elif self.aggr_method == 'hierarchical_mean':
                            cluster_feature = torch.mean(embeddings[indices_in_cluster], dim=0)
    ...
    def forward(self, visual_features_list=None, audio_features_list=None, bag_labels=None):
        if self.modality == 'visual':
            # [修改] 使用新的通用聚合函数
            v_bag, v_inst_logits, v_inst_embeds = self._aggregate_visual_features(visual_features_list)
            ...
            return {
                ...
                # 'v_bag_detached': v_bag.detach(),               
                # 'guided_v_proj': guided_v_proj                  
            }