...
class AudioGuidedAttention(nn.Module):
    ...
    def forward(self, visual_instances, audio_global):
        ...
        attn_scores = torch.matmul(Q, K.t()) / temp  # [1, N]
...
class LieDetection(nn.Module):
    ...
    def _compute_visual_base_attention(self, instance_embeddings_list):
        visual_base_attn_list = []
        for embeddings in instance_embeddings_list:
            sim_to_protos = torch.matmul(embeddings, self.visual_prototypes.t())
            instance_scores, _ = torch.max(sim_to_protos, dim=1)
            instance_weights = F.softmax(instance_scores, dim=0).unsqueeze(0)
            visual_base_attn_list.append(instance_weights)
        return visual_base_attn_list
    ...
    def forward(self, visual_features_list=None, audio_features_list=None, bag_labels=None):
        if self.modality == 'visual':
            ...
            visual_base_attn_list = self._compute_visual_base_attention(v_inst_embeds)
            ...
            return {
                'logits': logits,
                'visual_base_attention_weights': [w.detach() for w in visual_base_attn_list],
            ...
            raw_a_feat_norm = F.normalize(a_proj, p=2, dim=1)
            cos_sim_no_loss = torch.abs(torch.sum(v_feat_norm * raw_a_feat_norm, dim=1))

            return {
                'logits': logits,
                'cos_sim_no_loss': cos_sim_no_loss.detach(),
                'cross_attention_weights': attn_weights_list,
                'visual_base_attention_weights': [w.detach() for w in visual_base_attn_list],
                'ortho_loss': ortho_loss,
                'inst_logits': [l.detach() for l in v_inst_logits], # Added for visualization (Fig 3 & 5)
                'cos_sim': cos_sim.detach(),                        # Added for visualization (Fig 4)
            }