"""
NewData (NumberGuess) 数据集 - pplg_muti 非对称多模态模型训练脚本
5折交叉验证, 被试内 Pairwise Ranking + CE + alignment_loss
Top-K 评估 (被试级别)
"""
import os, glob
...
from models.pplg_mean import LieDetection
...
from utils.train import train_muti_newdata_epoch
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(" Starting NumberGuess pplg_muti Training (5-Fold CV)")
    ...
    for fold_idx in range(args.num_runs):
        ...
        # 元组级联比较: (Top-1, Top-2, Top-3, Top-5)
        ...
        for epoch in range(args.epochs):
            ...
            # 训练 (CE + Ranking + alignment_loss)
            train_metrics = train_muti_newdata_epoch(
            ...
            align_loss = train_metrics.get('align_loss', 0.0)
            fold_train_losses.append(current_train_loss)
            
            train_summary = (f"Fold {fold_idx+1} Epoch {epoch+1} Train: "
                             f"Loss={current_train_loss:.4f}, CE={ce_loss:.4f}, "
                             f"Rank={rank_loss:.4f}, Align={align_loss:.4f}")
            ...
            current_val_loss = 1.0 - topk_results[2]
    ...
    print("\nNumberGuess pplg_muti 5-Fold Cross-Validation complete. Final results logged.")
