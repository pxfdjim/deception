...
def _plot_kde_hist(sim_with_loss, sim_no_loss, stats, out_prefix):
    ...
    ax.hist(sim_with_loss, bins=bins, density=True, alpha=0.25, color='#1f77b4', label='Full AVPA')
...
def _plot_ecdf(sim_with_loss, sim_no_loss, stats, out_prefix):
    ...
    sns.ecdfplot(sim_with_loss, color='#1f77b4', linewidth=2.6, label='Full AVPA', ax=ax)
...
def _plot_violin_box(sim_with_loss, sim_no_loss, stats, out_prefix):
    ...
    labels = (['w/o Orthogonality Loss'] * len(sim_no_loss)) + (['Full AVPA'] * len(sim_with_loss))
    ...
    sns.violinplot(
        x=labels,
        y=values,
        order=['w/o Orthogonality Loss', 'Full AVPA'],
...
def _plot_paired_delta(sim_with_loss, sim_no_loss, stats, out_prefix):
    ...
    ax1.scatter(np.ones(sample_n), with_arr[sample_idx], color='#1F4788', s=6, alpha=0.25, label='Full AVPA')
    ax1.set_xticks([0, 1])
    ax1.set_xticklabels(['w/o Ortho', 'w/ Ortho'], fontsize=21, fontweight='bold')
    ax1.set_ylabel('Cosine Similarity', fontsize=24, fontweight='bold')
    ax1.tick_params(axis='y', labelsize=21)
    ax1.tick_params(axis='x', pad=10)  # 增加x轴标签与坐标轴的距离
    ax1.set_ylim(-0.55, 1.15)  # 扩大y轴上限，为图例留出空间
    # 创建图例，放在图表内右上角
    legend = ax1.legend(
        loc='upper right',
        ...
        fontsize=16,
        markerscale=3.5,
        labelspacing=0.8  # 增加图例项之间的间距
    )
    # 修改图例中的标记：去掉透明度，让颜色更鲜明
    for handle in legend.legend_handles:
        handle.set_alpha(1.0)  # 去掉透明度
        handle.set_sizes([80])  # 增大圆圈尺寸
    # 设置深黑色框线
    ...
    fig2, ax2 = plt.subplots(figsize=(8.5, 6.5))  # 进一步增大图表尺寸

    # 使用patches直接绘制柱状图，使用适度的透明度
    counts, bins, patches = ax2.hist(delta, bins=30, alpha=0.6, edgecolor='#1F4788', linewidth=0.8)
    # 强制设置每个柱子的填充颜色
    ...
    ax2.axvline(0.0, color='black', linestyle='--', linewidth=2.5, label='Zero')
    mean_val = np.mean(delta)
    ax2.axvline(mean_val, color='#1F4788', linestyle='--', linewidth=2.5, label='Mean $\\Delta$')
    ax2.set_xlabel('Δ Similarity', fontsize=34, fontweight='bold')
    ax2.set_ylabel('Count', fontsize=34, fontweight='bold')
    ax2.tick_params(axis='both', labelsize=30)
    ax2.tick_params(axis='x', pad=10)  # 增加x轴标签与坐标轴的距离
    ax2.tick_params(axis='y', pad=8)   # 增加y轴标签与坐标轴的距离
    # 设置深黑色框线
    ...
    fig2.tight_layout(pad=1.8)  # 适度padding避免超出边界
