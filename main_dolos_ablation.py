"""
DOLOS 数据集 - 多模态融合消融实验脚本
=====================================

运行所有消融配置，输出对比表格（包含每个fold详细指标）

用法:
    python main_dolos_ablation.py              # 运行所有消融模式
    python main_dolos_ablation.py --mode full  # 运行特定模式
"""
import os
import sys
import glob
...
def run_single_ablation(ablation_mode, args, logger, device):
    ...
    for fold_idx in range(args.num_runs):
        ...
        # 创建数据加载器
        train_loader, test_loader, _, _ = create_dolos_dataloaders(
        ...
        print(f"  Fold {fold_idx + 1} Best: ACC={best_fold_acc:.4f}, F1={best_fold_f1:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description='DOLOS Ablation Study')
    ...
    if all_results:
        ...
        print("\\begin{table}[h]")
        print("\\centering")
        print("\\caption{Ablation Study Results on DOLOS Dataset}")
        print("\\label{tab:ablation_dolos}")
